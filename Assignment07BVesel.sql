...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_BrittanyVesel')
	 Begin 
	  Alter Database [Assignment07DB_BrittanyVesel] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_BrittanyVesel;
	 End
	Create Database Assignment07DB_BrittanyVesel;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_BrittanyVesel;
...
--Select final columns, match on ProductID
...
--Change order by so dates are correct
SELECT ProductName, 
	[InventoryDate] = (DATENAME(mm,InventoryDate) + ', ' + DATENAME(Year,InventoryDate)), 
	Count
FROM vProducts
INNER JOIN vInventories ON vProducts.ProductID = vInventories.ProductID
ORDER BY ProductName, DatePart(month,InventoryDate);
go
...
-- categories links to products on CatID and products links to inventories on ProdID
...
--		InventoryDate,
--		Count,
--		ISNULL(LAG(Count) OVER (PARTITION BY ProductName ORDER BY DatePart(month,InventoryDate)),0) AS PreviousMonthCount
--FROM vProductInventories
--ORDER BY ProductName, DatePart(month,InventoryDate);
--go 
...
AS
	SELECT TOP 100000
		...
		ISNULL(LAG(Count) 
			OVER (PARTITION BY ProductName ORDER BY DatePart(month,InventoryDate)),0) 
